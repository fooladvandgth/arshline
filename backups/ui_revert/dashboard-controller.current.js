  ...
  if (tab === 'dashboard'){
        ...
        return; // IMPORTANT: prevent fall-through
            ...
            (function initMessagingReal(){
              ...
              function smsGetGroupFields(gid){ if(_smsGroupFieldsCache[gid]) return Promise.resolve(_smsGroupFieldsCache[gid]); if(_smsGroupFieldsInFlight[gid]) return _smsGroupFieldsInFlight[gid]; var p=fetch(ARSHLINE_REST+'groups/'+gid+'/fields',{ credentials:'same-origin', headers:{'X-WP-Nonce':ARSHLINE_NONCE} }).then(function(r){ if(!r.ok) throw new Error(r.status); return r.json(); }).then(function(arr){ var out=Array.isArray(arr)?arr.map(function(f){ return { name:String(f.name||'').trim(), label:String(f.label||'').trim()||String(f.name||'') }; }).filter(function(f){ return !!f.name; }):[]; _smsGroupFieldsCache[gid]=out; return out; }).finally(function(){ delete _smsGroupFieldsInFlight[gid]; }); _smsGroupFieldsInFlight[gid]=p; return p; }
              ...
              // Load groups
              fetch(ARSHLINE_REST + 'groups/list', { credentials:'same-origin', headers:{'X-WP-Nonce': ARSHLINE_NONCE} })
                .then(function(r){ if(!r.ok) throw new Error(r.status); return r.json(); })
                ...
                .catch(function(e){ console.warn('[ARSH][SMS] groups fetch failed', e); });
              // Load forms
